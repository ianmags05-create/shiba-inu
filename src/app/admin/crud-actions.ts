...
export async function updateImage(fd:FormData){await requireAdmin();const {error}=await createAdminClient().from("media_assets").update({name:text(fd,"name"),alt_text:text(fd,"alt_text"),slot_key:text(fd,"slot_key")||null}).eq("id",text(fd,"id"));if(error)throw new Error(error.message);refresh();}
